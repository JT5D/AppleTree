# The code below is from http://developer.amazonwebservices.com/connect/entry.jspa?externalID=135&categoryID=47
require 'base64'
require 'cgi'
require 'openssl'
require 'digest/sha1'
...
class AWS
  ...
  def initialize(access_key, secret_key, bucket_name, public_read = true)
    @access_key, @secret_key, @bucket = access_key, secret_key, bucket_name
  ...
  def bucket_exists?
    @connection.check_bucket_exists(bucket)
  end
  
  def upload(file_path)
    content_type = `file -bi \"#{file_path}\"`.strip
    basename = File.basename(file_path)
    
    @connection.put(@bucket, basename, 
      S3Object.new(File.open(file_path).read, {
        "uploaded-from" => "Dropzone AWS"
      }), {
      'Content-Type' => content_type
    }.merge(true ? {'x-amz-acl' => 'public-read'} : {}))
    
    if public_read?
      generator = QueryStringAuthGenerator.new(@access_key, @secret_key)
      url = generator.get(@bucket, basename).gsub(/\?.*$/, '')
    else
      false
    end
  end
  ...
  def connect!
    @connection = AWSAuthConnection.new(@access_key, @secret_key)
  end
end